...
var Select = React.createClass({
	...
	propTypes: {
		...
		menuLength: React.PropTypes.number, // maximum number of options in menu
   ...
   */
	...
	getStateFromValue: function getStateFromValue(value, options) {
		...
		return {
			...
			values: values,
			inputValue: values.length ? values[0].label : "",
			filteredOptions: filteredOptions,
			placeholder: this.props.placeholder,
			focusedOption: values.length ? values[0] : filteredOptions[0]
		};
	},
	...
	filterOptions: function filterOptions(options, values) {
		var filterValue = this._optionsFilterString.toLowerCase(),
			filterValueLength = filterValue.length,
			filterFunc;

		if ( filterValueLength === 0 ) return options;

		if ( this.props.matchPos === "start" ) {
			filterFunc = function (op) {
				var labelTest = String(op.label).toLowerCase().substr(0, filterValueLength);
				return labelTest === filterValue
			};
		} else {
			filterFunc = function (op) {
				var labelTest = String(op.label).toLowerCase().indexOf(filterValue);
				return labelTest !== -1;
			};
		}

		return _.filter(options, filterFunc, this);
	},
	...
	buildMenu: function buildMenu() {
		...
		if (this.state.filteredOptions.length > 0) {
			...
			if (this.props.menuLength) {
				this.state.filteredOptions = this.state.filteredOptions.slice(0, this.props.menuLength);
			}
		}
		...
		return ops.length ? ops : React.createElement(
			...
			!this.state.inputValue ? this.props.searchPromptText : this.props.noResultsText
		);
	},
	...
	render: function render() {
		...
		var clear = this.props.clearable && this.state.value && !this.props.disabled ?
			React.createElement("span", {
				...
				title: this.props.clearValueText,
				"aria-label": this.props.clearValueText,
